	...
	$('#fpmLogin').submit(function(e) {
	 ...
	 $.ajax({
	     ...
	     data: { form: 'login', username: '' + username + '', password: '' + password + ''}
		     ...
		     if(msg == 'Logging into Developer Panel...') {
		    	 setTimeout(function(){window.location='developer'},3000); 
		     } else {
...
//create user
	$('#create-account').submit(function(e) {
		var email	    = $("#create-email").val();
			...
			phone	    = $("#create-phone-number").val();
			role	    = $("#create-role").val();
			company		= $("#create-company").val();
			password    = $("#create-password").val();
			confirm     = $("#create-confirm").val();
		...
		if(password == confirm && password != '' && password.length >= 6) {
		 $.ajax({
		     type: "POST",
		     url: 'submit.php',
		     data: { form: 'create-account', first_name: '' + first_name + '', last_name: '' + last_name + '', phone: '' + phone + '', role: '' + role + '', email: '' + email + '', company: '' + company + '', password: '' + password + '' }
		   }).success(function( msg ) {
			      $('.register_success').css("display", "");
			      $(".register_success").fadeIn(1000, "linear");
			      $('.register_success_text').fadeIn("slow");
...
//add project
	$('#add-project').submit(function(e) {
		var project	    = $("#project-name").val();
			description	= $("textarea#project-description").val();
	 ...
	 $.ajax({
	     type: "POST",
	     url: 'submit.php',
	     data: { form: 'project-add', project: '' + project + '', description: '' + description + '', type: '' + type + '', company: '' + company + '' }
	   }).success(function( msg ) {
		      $('.success').css("display", "");
		      $(".success").fadeIn(1000, "linear");
		      $('.success_text').fadeIn("slow");
